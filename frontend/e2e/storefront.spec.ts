...
test.describe("the public storefront", () => {
  ...
  // D25: the stored tenant accent is accepted but no longer rendered. Both demo
  // tenants have one, so a re-injected override would show as a non-Sakura fill.
  test("the stored tenant accent does not recolor the chat entry", async ({ page }) => {
    for (const tenant of [
      ...
      expect(background).toBe("rgb(141, 42, 88)");
    }
  });
