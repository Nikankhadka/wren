...
No DB/LLM needed: `build_graph(supervisor_node=...)` lets tests force a
route without real intent classification (T-013), and `stream_mode="updates"`
reports exactly which node produced each state delta, so the test observes
the actual execution order rather than asserting on wiring it can't see.
"""
...
async def _run_and_collect_node_order(route: str) -> list[str]:
    ...
    context = GraphContext(tenant_id=uuid4(), provider=BaseFakeProvider(), reranker=FakeReranker())
    order: list[str] = []
